...
const syncingDauMauInDb = async (event, context) => {
  try {
    ...
    let [currDateData, prevDateData, currMonthData, prevMonthData] = await Promise.all([getRedisSet(`${redisPrefixForDau}-${currDate}`), getRedisSet(`${redisPrefixForDau}-${prevDate}`), getRedisSet(`${redisPrefixForMau}-${currMonth}`), (currMonth != prevMonth) ? getRedisSet(`${redisPrefixForMau}-${prevMonth}`) : '']);
