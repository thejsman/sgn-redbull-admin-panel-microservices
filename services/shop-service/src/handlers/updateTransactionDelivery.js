...
import { getTransactionsByTransactionId, updateTransaction, getGiftsByGiftId, updateGifts, updateDeliveryObjectAndWithGiftsArray } from "../lib/transactionsHelper";

const updateTransactionDelivery = async (event, context) => {
    try {
        const { transactionId, deliveryDate, transactionType = "deals", userId } = JSON.parse(event.body);
        try {
            if (transactionType === "deals") {
                ...
                if (data && data.Items.length) {
                    ...
                    let response = responseHandler({
                        ...
                        data: { transactionObj: data && data.Items.length ? data.Items[0] : {} },
                    });
                    return response;
                }
                else {
                ...
                return Promise.all([transactionData, giftData])
                    .then(function (result) {
                        ...
                        if (result[0].Items.length && result[1].Items.length) {
                            ...
                            let updateDeliveryObjectInTransaction = updateDeliveryObjectAndWithGiftsArray({ deliveryTransactionObject, transactionId, userId: transaction.userId, giftWith });
                            let updateDeliveryObjectInGifts = updateGifts({ deliveryGiftObject, transactionId, userId: gift.userId });

                            return Promise.all([updateDeliveryObjectInTransaction, updateDeliveryObjectInGifts])
                                .then(function (result) {
                                    let response = responseHandler({
                                        statusCode: 200,
                                        message: "Transactions Object",
