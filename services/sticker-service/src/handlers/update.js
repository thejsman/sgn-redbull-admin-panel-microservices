...
const updateSticker = async (event, context) => {
  try {
    ...
    if (data.base64) {
      ...
      try {
        ...
        //Now update uploadIMage location with cdn url
        if (process.env.CDN_BUCKET_URL) {
          uploadImage.Location = process.env.CDN_BUCKET_URL + uploadImage.Key;
        }
...
export const handler = updateSticker;
